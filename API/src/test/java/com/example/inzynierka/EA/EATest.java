...
import com.example.inzynierka.tunningControllers.DMCAnalityczny;
import com.example.inzynierka.tunningControllers.PID;
...
class EATest {
    ...
    void PIDSisoTest() {
        ...
        double[] tempWartosciGA = GA.getParameters(controllerTunning.getNumberOfTuningParameters(), controllerTunning, SISODPA);
    ...
    void PIDMimoTest() throws IOException {
        ...
        assert (GA.getMutationNumber() == GA.getMu() - GA.getRecombinationNumber());
        assert (GA.getMutationprobability() == 0.3);
        double[] tempWartosciGA = GA.getParameters(controllerTunning.getNumberOfTuningParameters(), controllerTunning, obiekt);
        assert (tempWartosciGA[0] != 0.0);
        assert (tempWartosciGA.length == 3);
    }
