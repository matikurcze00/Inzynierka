...
public class DPA {
    ...
    public DPA(double gain, double R1, int Q1, double R2,
        ...
        checkArgs(R1, Q1, R2, Q2, T1, T2, T3, Tp);
    ...
    private static void setDenomDeg(double R1, double R2, double T1, double T2, double T3, int nomDeg) {
        int denomDeg = 0;
        if (T1 > 0) {
            denomDeg++;
        }
    ...
    private static void checkValueQ(double Q1, double Q2) {
        if (Q1 != -1 && Q1 != 0 && Q1 != 1) {
    ...
    public double simulateStep(List<Double> u) {
    ...
    private void checkArgs(double R1, double Q1, double R2, double Q2, double T1, double T2, double T3, double Tp)
        ...
        int nomDeg = 0;
        setDenomDeg(R1, R2, T1, T2, T3, nomDeg);
        checkValueQ(Q1, Q2);
        checkValueRT(R1, R2, T1, T2);
    }

    private void checkValueRT(double R1, double R2, double T1, double T2) {
        if (T1 <= 0 && R1 > 0) {
